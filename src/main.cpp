    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          // either we will reference the starting point as where the car is or at the previous poths end point
          double ref_x = car_x;
          double ref_y = car_y;
...
//          }
          ...
          /*
          // DEBUG : Initial code test
          double dist_inc = 0.3;          // default value : 0.5; with that waypoints are expected to be 0.5 m apart, i.e. moving at approx 50 mph
          for (int i = 0; i < 50; ++i) {
            double next_s = car_s + (i + 1) * dist_inc;
            double next_d = 6;
            vector<double> xy = getXY(next_s, next_d, map_waypoints_s, map_waypoints_x, map_waypoints_y);
            next_x_vals.push_back(xy[0]);                                       // DEBUG : drive within lane
            next_y_vals.push_back(xy[1]);                                       // DEBUG : drive within lane
            // next_x_vals.push_back(car_x+(dist_inc*i)*cos(deg2rad(car_yaw)));    // DEBUG : drive straight
            // next_y_vals.push_back(car_y+(dist_inc*i)*sin(deg2rad(car_yaw)));    // DEBUG : drive straight
          }
          */
